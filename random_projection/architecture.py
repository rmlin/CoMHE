...
class VGG():
    ...
    def _add_thomson(self, filt, n_filt):
    ...
    def _add_thomson_constraint(self, oldfilt, ksize, n_input, oldn_filt, pd, pn, pnd):
        pd1 = pd
    ...
    def _add_thomson_constraint_final(self, filt, n_filt):
    ...
    def _conv_layer(self, bottom, ksize, n_filt, is_training, pd, pn, pnd, name, stride=1, 
        ...
        with tf.variable_scope(name) as scope:
            ...
            if thom == 'True':
                if n_input == 3:
                    self._add_thomson(filt, n_filt)
                else:                  
                    self._add_thomson_constraint(filt, ksize, n_input, n_filt, pd, pn, pnd)
            elif thom == 'Final':
                self._add_thomson_constraint_final(filt, n_filt)
