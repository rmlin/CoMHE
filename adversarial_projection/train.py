...
def train(base_lr, batch_sz, gpu_no, pd, pn, iters):
    ...
    ploss_op = -tf.add_n(p_loss_list) #negative loss here in order to maximize the energy 
    ...
    updatep_op = tf.train.GradientDescentOptimizer(lr_).minimize(ploss_op, var_list=p_vars)  #maximize the projected thomson loss
    update_op = tf.train.MomentumOptimizer(lr_, 0.9).minimize(loss_op, var_list=normal_vars) #minimize the maximized thomson loss 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--random_seed', type=int, default=3456,
                    help='random seed')
